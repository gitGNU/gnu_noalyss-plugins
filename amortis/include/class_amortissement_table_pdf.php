...
class Amortissement_Table_PDF extends PDFLand
{
    ...
    function export()
    {
        ...
        where a_id=$1 and ad_year=$2", array($array[$i]['a_id'], $this->year));
            ...
            $this->write_cell($col_size['code'], 8, $fiche->strAttribut(ATTR_DEF_QUICKCODE), 0, 0, 'L', $fill);
            $this->write_cell($col_size['desc'], 8, $fiche->strAttribut(ATTR_DEF_NAME), 0, 0, 'L', $fill);
            $this->write_cell($col_size['date.purch'], 8, format_date($array[$i]['a_date']), 0, 0, 'L', $fill);
            $this->write_cell($col_size['year.purch'], 8, $array[$i]['a_start'], 0, 0, 'C', $fill);
            $this->write_cell($col_size['amount.purch'], 8, nb($array[$i]['a_amount']), 0, 0, 'R', $fill);
            $this->write_cell($col_size['#amort'], 8, nb($array[$i]['a_nb_year']), 0, 0, 'R', $fill);
            $this->write_cell($col_size['amount.amort'], 8, nb($remain), 0, 0, 'R', $fill);
            $this->write_cell($col_size['%'], 8, nb($pct), 0, 0, 'R', $fill);
            $this->write_cell($col_size['%'], 8, nb($amortize), 0, 0, 'R', $fill);
            $this->write_cell($col_size['amount.remain'], 8, nb($toamortize), 0, 0, 'R', $fill);
            $this->line_new();
        }
        $this->line_new(10);
        $tot=$this->cn->get_value(" select coalesce(sum(a_amount),0) from amortissement.amortissement where a_start=$1", array($this->year));
        $this->setX(40);
        $this->write_cell(60, 8, "Acquisition de l'année", 1, 0, 'R');
        $this->write_cell(60, 8, nb($tot), 1, 0, 'R');
        $this->line_new();

        $this->setX(40);
        $this->write_cell(60, 8, "Amortissement", 1, 0, 'R');
        $this->write_cell(60, 8, nb($tot_amort), 1, 0, 'R');
        $this->line_new();

        $this->setX(40);
        $this->write_cell(60, 8, "Valeur net", 1, 0, 'R');
        $this->write_cell(60, 8, nb($tot_net), 1, 0, 'R');
        $this->line_new();
