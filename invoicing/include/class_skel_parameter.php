...
class SKEL_Parameter
{
    ...
    function save($p_option,$p_value)
    {
        ...
        if ( $this->cn->get_value('select count(*) from SKEL.parameter where pr_id=$1',array($p_option)) != 0 )
        {
            $Res=$this->cn->exec_sql("update SKEL.parameter set pr_value=$1 where pr_id=$2",
                                     array($p_value,$p_option));
        }
        else
        {

            $Res=$this->cn->exec_sql("insert into SKEL.parameter (pr_id,pr_value) values( $1,$2)",
