...
class Impacc_Operation
{
    ...
    function __construct()
    {
        ...
        $this->encoding="utf-8";
        $this->filename="";
        $this->date_transfert=null;
        $this->date_upload=date('Y.m.d HH:MM');
        $this->decimal='.';
        $this->thousand='';
    }
    ...
    function record()
    {
        ...
        while (($row=fgetcsv($foperation, 0, ";", '"'))!==false)
        {
            ...
            {
                $str_row=implode($row, ";");
                printf(_("Attention % ne contient pas 11 colonnes"), h($str_row));
                continue;
            }
        ...
        printf(_("Nombre de lignes enregistrées : %d"), $this->row_count);
    ...
    function check()
    {
        ...
        {
            $cn->start();
            $array=$cn->get_array("select o_id from impacc.operation_tmp where i_id=$1 order by o_id",
                    array($this->impid));
            $nb_row=count($array);
            for ($i=0; $i<$nb_row; $i++)
            {
                 ...
                 * detect duplicate
                 */
                $db=$cn->get_value("select count(*) from impacc.operation_tmp where o_doli=$1 and o_id in (select o_id from impacc.operation_transfer)",
                        array($dol));
                if ($db>0)
                {
                ...
                if (trim($poste)==''||$cn->get_value("select count(*) from tmp_pcmn where pcm_val=$1",
                                array($poste))==0)
                {
                    $operation->setp("code", 'N');
                ...
                $a=array("rate"=>" Taux de TVA", "amount_total"=>"Montant total",
                    "number_unit"=>'Nombre d\'unité', "amount_vat"=>"Montant TVA");
                ...
                {
                    $tva_id=$cn->get_array("select tva_id from impacc.parameter_tva where pt_rate/100=$1",
                ...
                $nb_customer=$cn->get_value("select count(*) from impacc.operation_tmp where o_type='T' and o_doli=$1 and i_id=$2",
                        array($code_op, $this->impid));
                $nb_good=$cn->get_value("select count(*) from impacc.operation_tmp where o_type='S' and o_doli=$1 and i_id=$2",
                        array($code_op, $this->impid));
                if ($nb_customer==0)
                {
                    $operation->setp("code", 'N');
    ...
    function transfer()
    {
        ...
        try
        {
            ...
            for ($i=0; $i<$nb_row; $i++)
            {
                 ...
                 */
                $adetail=$cn->get_array("select o_id from impacc.operation_tmp where o_doli=$1 and i_id=$2 and o_type='S'",
                        array($array[$i]['o_doli'], $this->impid));
                $atiers=$cn->get_array("select o_id from impacc.operation_tmp where o_doli=$1 and i_id=$2  and o_type='T'",
                        array($array[$i]['o_doli'], $this->impid));
                if (count($atiers)>1)
                {
                ...
                for ($e=0; $e<$nb_detail; $e++)
                {
                    ...
                    $date=format_date($oper->getp("date"), "YYYY-MM-DD",
                            "DD.MM.YYYY");
                    $oper->setp("date", $date);
                    ...
                    switch ($type)
                    {
                        case 'ACH':
                            ...
                            $sum_side=($save_amount>0)?bcadd($sum_side,
                                            $amount_tvac):$sum_side;
                            break;
                        case 'VEN':
                    ...
                    {
                        $tva[$tva_id][$side]=bcadd($tva[$tva_id][$side],
                                $amount_tva);
                    }
                    else
                    {
                ...
                $acc_jrn->desc=mb_substr($oper_tiers->getp("desc"), 0, 80,
                        'UTF8');
                $acc_jrn->date=$date;
                $acc_jrn->grpt=$grpt;
                $acc_jrn->periode=0;
                ...
                $cn->exec_sql('update jrn set jr_pj_number=$1 where jr_id=$2',
                        array($oper->getp('pj'), $acc_jrn->jr_id));
                ...
                {
                    case 'ACH':
                        $cn->exec_sql('update quant_purchase set qp_internal = $1 where j_id in (select j_id from jrnx where j_grpt=$2)',
                                array($internal, $grpt));
                        break;
                    case 'VEN':
                        $cn->exec_sql('update quant_sold set qs_internal = $1 where j_id in (select j_id from jrnx where j_grpt=$2)',
                                array($internal, $grpt));
                        break;
                }
            }// loop i
    ...
    function result_transfer()
    {
        ...
        for ($i=0; $i<count($arow); $i++)
        {
            ...
            echo '<td>'.HtmlInput::detail_op($arow[$i]['jr_id'],
                    $arow[$i]['jr_internal']).'</td>';
            echo '</tr>';
        }
        echo '</table>';
    }
    ...
    {
        global $cn;
        $in_delimiter=new IText('in_delimiter',$this->delimiter);
        $in_delimiter->size=1;
        ...
        $in_filename=new IFile('in_filename');
